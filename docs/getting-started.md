...
- Every tab is a path of its own: `/library`, `/tags`, `/faces`, `/history`, `/train`, `/evaluate`, `/models`. The bare root redirects to `/library`, and the Tags tab's other two lists are a second path segment (`/tags/links` for the meta tags, `/tags/tagsets` for a tag set, which adds `?set=` for the set that is open). The three retired sub-tab addresses (`/tags/subjects`, `/tags/places`, `/tags/events`) still land on the tag list narrowed to that kind.
