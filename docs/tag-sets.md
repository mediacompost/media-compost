...
A built-in is **read-only**. You cannot rename it, add or remove an entry, or delete it — it is the app's list, not yours, and *Duplicate* is the copy that is. The quickest way to one is **Make editable**, in the place Add category has on an editable set: it makes the copy, puts it right after the built-in in the list, switches it on and the built-in off, and opens it — so the names on offer stay the same and only now you can change them. The built-in stays in the list, switched off. What you *can* set is everything that is a fact about **this library** rather than about the file: whether it is switched on, where it sits in the list, and the two switches in **Properties…** — *Offer alias spellings* and *Create implied tags*. It exports like any other set, switched on or not.
