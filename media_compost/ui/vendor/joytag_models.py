# Vendored verbatim from JoyTag (https://github.com/fpgaminer/joytag),
# file Models.py, by fpgaminer. Licensed under the Apache License 2.0.
#
# JoyTag ships this model code only in its GitHub repo, not in the Hugging Face
# weights repo (fancyfeast/joytag), so we vendor it here to load the published
# weights (config.json + model.safetensors + top_tags.txt) without requiring a
# separate git clone. Kept byte-for-byte except for this header; do not edit —
# re-sync from upstream if updating.
# ---------------------------------------------------------------------------
import json
