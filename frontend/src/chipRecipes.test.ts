/**
 * A SMALL LABELLED BOX IS `shared/Chip`, and the inline `letterSpacing:`
 * recipes that remain — the tracked uppercase marks that are neither a
 * chip nor a section heading — are a stated budget, strict both ways.
 */
...
const BUDGET: Record<string, number> = {
  ...
  "train/ArchitectureMap.tsx": 2,
  "train/DegradeSection.tsx": 1,
  "train/TrainView.tsx": 3,
};

test("every inline tracked label is in the budget, and the budget is exact", () => {
  const actual: Record<string, number> = {};
  for (const dir of ["app", "train", "shared", "query"]) {
    for (const p of walk(join(SRC, dir))) {
      const rel = p.slice(SRC.length);
      if (EXEMPT.has(rel)) continue;
      const n = (readFileSync(p, "utf8").match(/letterSpacing:/g) ?? []).length;
      if (n) actual[rel] = n;
    }
  }
  ...
  for (const [f, n] of Object.entries(actual)) {
    const b = BUDGET[f] ?? 0;
    if (n > b) problems.push(`${f}: ${n} tracked labels, budget ${b} — use <Chip> / <SectionHeading>`);
    if (n < b) problems.push(`${f}: ${n} tracked labels, budget ${b} — lower the budget`);
  }
